...
import {List,TextareaItem,WhiteSpace,Toast,WingBlank ,InputItem,ImagePicker,Checkbox} from 'antd-mobile'
...
@withRouter
class FastDoctor extends React.Component{
    constructor(props){
        super(props)
        this.state = {
            problemContent:'',
            payUrl:'',
            money:JSON.parse(this.props.match.params.data).max,
            departmentcode:JSON.parse(this.props.match.params.data).code,
            imgArr:[],
            files:[],
            shareValue:true,
            share:1
    ...
    handleClick=()=>{
        ...
        httpPost('/Qa/addProblem',{problem:this.state.problemContent,is_urgent:0,typeid:2,share:this.state.share,dementcode:this.state.departmentcode,money:this.state.money},{imgurl:Array.from(new Set(this.state.imgArr))}).then(res=>{
            console.log(res)
        ...
        return (
            <div>
                ...
                <List className="my-list">
                    <WingBlank>
                        <WhiteSpace/>
                        <p style={{margin:'.07rem 0'}}>搜索关键词：婴儿发热</p>
                        <p style={{margin:'.07rem 0'}}>医生职称：主任医师</p>
                        <p style={{margin:'.07rem 0'}}>医生科室：儿科</p>
                        <p style={{margin:'.07rem 0'}}>医院等级：三级医院</p>
                        <WhiteSpace/>
                    </WingBlank>
                </List>
                <WhiteSpace/>
                <List>
                    <InputItem
                        type='number'
                        placeholder={this.state.money?this.state.money:<div>请输入咨询价格</div>}
                        className='setting-consult'
                        extra="¥"
                        onBlur={v=>this.onBlurInput('ur',v)}
                    >咨询价格(元)</InputItem>
                </List>
                <WhiteSpace/>
                {/*提问内容*/}
